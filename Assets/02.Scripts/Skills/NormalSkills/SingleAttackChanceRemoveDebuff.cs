using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class SingleAttackChanceRemoveDebuff : ISkillEffect
{
    private SkillData skillData;
    
    public SingleAttackChanceRemoveDebuff(SkillData data)
    {
        skillData = data;
    }
    
    public IEnumerator Execute(Monster caster, List<Monster> targets)
    {
        if (skillData == null || targets == null || targets.Count == 0) yield break;

        var targetCopy = new List<Monster>(targets);
        
        foreach (var target in targetCopy)
        {
            var result = DamageCalculator.CalculateDamage(caster, target, skillData);
            int damage = Mathf.RoundToInt(result.damage * 0.4f);
            
            BattleManager.Instance.DealDamage(target, damage, caster, this.skillData, result.isCritical, result.effectiveness);
        }
        
        if (Random.value < 0.1f && caster.Level >= 10 && caster.ActiveStatusEffects.Count > 0)
        {
            int randomRange = Random.Range(0, caster.ActiveStatusEffects.Count);
            caster.ActiveStatusEffects.RemoveAt(randomRange);
        }
    }
}
